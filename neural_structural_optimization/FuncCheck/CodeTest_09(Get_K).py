...
print("x_list",x_list,x_list.size)
print("y_list",y_list,y_list.size)
print("value_list",value_list,value_list.size)
...
e = 1
nu = 0.3
stiffness = np.array([[[0,1,2,3,4,5,6,7],[0,1,2,3,4,5,6,7],[0,1,2,3,4,5,6,7],[0,1,2,3,4,5,6,7],[0,1,2,3,4,5,6,7],[0,1,2,3,4,5,6,7],[0,1,2,3,4,5,6,7],[0,1,2,3,4,5,6,7]],
...
ik = Iar[0,:]
jk = Iar[1,:]
print(ik,ik.max(),ik.shape[0])
print(jk,jk.max(),jk.shape[0])
...
print(type(kd),kd.shape)

print(nDof)
print(ik.size)
print(math.ceil(ik.size/nDof))
a = math.ceil(ik.size/nDof)

  #value_list = (kd * np.tile(ke, kd.shape)).flatten()
value_list = scipy.sparse.coo_matrix((kd,(ik,jk)),[a*nDof,a*nDof]).toarray().flatten()
