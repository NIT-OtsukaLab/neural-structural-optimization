...
class Problem:
  ...
  Attributes:
	...
	depth: integer depth of the domain.		2020-12-07 K.Taniguchi
  ...
  def __post_init__(self):
    ...
    if (isinstance(self.mask, np.ndarray)
        and self.mask.shape != (self.height, self.width, self.depth)):	#2020-12-07 K.Taniguchi
...
def cantilever_beam_full(
    width=60, height=60, depth=60, density=0.5, force_position=0):
...
def cantilever_beam_two_point(
    width=60, height=60, depth=60, density=0.5, support_position=0.25,
    force_position=0.5):
...
def pure_bending_moment(
    width=60, height=60, depth=60, density=0.5, support_position=0.45):
  ...
  normals[0, round(height*(1-support_position)), :, Z] = 1
  normals[0, round(height*support_position), :, Z] = 1

  forces = np.zeros((width + 1, height + 1, depth + 1, 3))
  forces[0, round(height*(1-support_position)), round(depth/2), X] = 1
  forces[0, round(height*support_position), round(depth/2), X] = -1

  return Problem(normals, forces, density)

#2020-12-07 K.Taniguchi
def michell_centered_both(width=32, height=32, depth=32, density=0.5, position=0.05):
  """A single force down at the center, with support from the side."""
  # https://en.wikipedia.org/wiki/Michell_structures#Examples
  normals = np.zeros((width + 1, height + 1, depth + 1, 3))
  normals[round(position*width), round(height/2), round(depth/2), Y] = 1
  normals[round(position*width), round(height/2), round(depth/2), Z] = 1
  normals[-1, :, :, X] = 1

  forces = np.zeros((width + 1, height + 1, depth + 1, 3))
  forces[-1, round(height/2), round(depth/2), Y] = -1

  return Problem(normals, forces, density)

#2020-12-07 K.Taniguchi
def michell_centered_below(width=32, height=32, depth=32, density=0.5, position=0.25):
  """A single force down at the center, with support from the side below."""
  # https://en.wikipedia.org/wiki/Michell_structures#Examples
  normals = np.zeros((width + 1, height + 1, depth + 1, 3))
  normals[round(position*width), 0, :, Y] = 1
  normals[round(position*width), 0, :, Z] = 1
  normals[-1, :, :, X] = 1
...
#2020-12-07 K.Taniguchi
def ground_structure(width=32, height=32, depth=32, density=0.5, force_position=0.5):
...
#2020-12-07 K.Taniguchi
"""
def l_shape(width=32, height=32, depth=32, density=0.5, aspect=0.4, force_position=0.5):
...
#2020-12-07 K.Taniguchi
"""
def crane(width=32, height=32, depth=32, density=0.3, aspect=0.5, force_position=0.9):
...
#2020-12-07 K.Taniguchi
def tower(width=32, height=32, depth=32, density=0.5):
...
#2020-12-07 K.Taniguchi
def center_support(width=32, height=32, depth=32, density=0.3):
...
#2020-12-07 K.Taniguchi
def column(width=32, height=32, depth=32, density=0.3):
...
#2020-12-07 K.Taniguchi
def roof(width=32, height=32, depth=32, density=0.5):
...
#2020-12-07 K.Taniguchi
def causeway_bridge(width=60, height=20, depth=20, density=0.3, deck_level=1):
...
#2020-12-07 K.Taniguchi
def two_level_bridge(width=32, height=32, depth=32, density=0.3, deck_height=0.2):
  ...
  forces = np.zeros((width + 1, width + 1, depth + 1, 3))
  forces[:, round(height * (1 - deck_height) / 2), round(depth/2), :] = -1 / (2 * width)
  forces[:, round(height * (1 + deck_height) / 2), round(depth/2), :] = -1 / (2 * width)
  return Problem(normals, forces, density)

#2020-12-07 K.Taniguchi
def suspended_bridge(width=60, height=20, depth=20, density=0.3, span_position=0.2, anchored=False):
  """A bridge above the ground, with supports at lower corners."""
  normals = np.zeros((width + 1, height + 1, depth + 1, 3))
  normals[-1, :, :, X] = 1
  normals[:round(span_position*width), -1, round(depth/2), Y] = 1
  normals[:round(span_position*width), -1, round(depth/2), Z] = 1
  if anchored:
...
#2020-12-07 K.Taniguchi
def canyon_bridge(width=60, height=20, depth=20, density=0.3, deck_level=1):
  ...
  forces = np.zeros((width + 1, height + 1, depth + 1, 3))
  forces[:, deck_height, round(depth/2), Y] = -1 / width
  return Problem(normals, forces, density)

#2020-12-07 K.Taniguchi
def thin_support_bridge(
    width=32, height=32, depth=32, density=0.25, design_width=0.25):
...
#2020-12-07 K.Taniguchi
def drawbridge(width=32, height=32, depth=32, density=0.25):
  """A bridge supported from above on the left."""
  normals = np.zeros((width + 1, height + 1, depth + 1, 3))
  normals[0, :, :, :] = 1

  forces = np.zeros((width + 1, height + 1, depth + 1, 3))
  forces[:, -1, round(depth/2), Y] = -1 / width

  return Problem(normals, forces, density)

#2020-12-07 K.Taniguchi
def hoop(width=32, height=32, depth=32, density=0.25):
...
def multipoint_circle(
    width=140, height=140, depth=140, density=0.333, radius=6/7,
  ...
  c_z = depth // 2
  normals = np.zeros((width + 1, height + 1, depth + 1, 3))
  normals[c_x - 1 : c_x + 2, c_y - 1 : c_y + 2, c_z - 1 : c_z + 2, :] = 1
  assert normals.sum() == 18

  c1, c2, c3, c4, c_x0, c_y0, c_z0 = weights
  ...
  for position in range(num_points):
    ...
    j = int(round(c_y + y))
	k = int(round(c_z + z))
    forces[i, j, k, X] = + c1 * y + c2 * x + c3 * y + c4 * x + c_x0
    forces[i, j, k, Y] = - c1 * x + c2 * y + c3 * x - c4 * y + c_y0
    forces[i, j, k, Z] = ...

  return Problem(normals, forces, density)
"""
#2020-12-07 K.Taniguchi
def dam(width=32, height=32, depth=32, density=0.5):
...
#2020-12-07 K.Taniguchi
def ramp(width=32, height=32, depth=32, density=0.25):
  """Support downward forces on a ramp."""
  return staircase(width, height, depth, density, num_stories=1)

#2020-12-07 K.Taniguchi
def staircase(width=32, height=32, depth=32, density=0.25, num_stories=2):
...
#2020-12-07 K.Taniguchi
def staggered_points(width=32, height=32, depth=32, density=0.3, interval=16, break_symmetry=False):
...
#2020-12-07 K.Taniguchi
def multistory_building(width=32, height=32, depth=32, density=0.3, interval=16):
...
PROBLEMS_BY_CATEGORY = {
	...
	"""
    'l_shape_0.2': [
        l_shape(64, 64, 64, aspect=0.2, density=0.4),
        l_shape(128, 128, 128, aspect=0.2, density=0.3),
        l_shape(256, 256, 256, aspect=0.2, density=0.2),
    ],
    'l_shape_0.4': [
        l_shape(64, 64, 64, aspect=0.4, density=0.4),
        l_shape(128, 128, 128, aspect=0.4, density=0.3),
        l_shape(256, 256, 256, aspect=0.4, density=0.2),
    ],
    'crane': [
        crane(64, 64, 64, density=0.3),
        crane(128, 128, 128, density=0.2),
        crane(256, 256, 256, density=0.15),
        crane(256, 256, 256, density=0.1),
    ],
	"""
    ...
    'hoop': [
        hoop(32, 64, 64, density=0.25),
        hoop(64, 128, 128, density=0.2),
        hoop(128, 256, 256, density=0.15),
    ],
    'dam': [
        dam(64, 64, 64, density=0.2),
        dam(128, 128, 128, density=0.15),
        dam(256, 256, 256, density=0.05),
        dam(256, 256, 256, density=0.1),
        dam(256, 256, 256, density=0.2),
    ],
    'ramp': [
        ramp(64, 64, 64, density=0.3),
        ramp(128, 128, 128, density=0.2),
        ramp(256, 256, 256, density=0.2),
        ramp(256, 256, 256, density=0.1),
    ],
    'staircase': [
        staircase(64, 64, 64, density=0.3, num_stories=3),
        staircase(128, 128, 128, density=0.2, num_stories=3),
        staircase(256, 256, 256, density=0.15, num_stories=3),
        staircase(128, 512, 512, density=0.15, num_stories=6),
    ],
	"""
    'staggered_points': [
